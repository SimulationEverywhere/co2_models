...
#Hoda: hardcoded temporarily to try the model concept
    vent_val = 51.8, 73.7, 84.7
    ...
    for i in range(im_res.size[0]):
        ...
        for j in range(im_res.size[1]):
            ...
            else if almost_equal(pixels[i, j][:3], vent_color[:3], args.tolerance):
                pixels[i, j] = COLOR_VENT 
                mat_id[-1].append(vent_val)
            else:
